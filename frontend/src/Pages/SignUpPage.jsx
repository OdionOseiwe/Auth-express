    ...
    className='max-w-md w-full bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden'>
      <div className='p-8'>
        ...
        <form onSubmit={handleSignUp}>
            <Input icon={User} type="text" placeholder="Full Name" value={name} 
            onChange={(e)=> setName(e.target.value)}/ >
                
            <Input icon={Mail} type="text" placeholder="Email address" value={email} 
            ...
            whileTap={{scale:0.98}}>
                {isLoading ? <Loader className='animate-spin mx-auto'></Loader>: "Sign Up"}
            </motion.button>
        </form>
      </div>
      <div className='px-8 py-4 bg-gray-900 bg-opacity-50 flex justify-center  '>
        <p className='text-sm text-gray-400'>
          ...
          <Link to={"/login"} className='text-green-400 px-4 underline'>Login</Link>
        </p>
      </div>
    </motion.div>
  )
